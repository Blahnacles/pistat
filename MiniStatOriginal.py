...
#  2018 Modified by Grant van Riessen for additional function: deposition/stripping
#
#
...
pd_parameters = {} # Dictionary to hold the pulsed deposition parameters
...
def ramp(slope,time,limit):
    """ Return value along straight line (=slope*time) up to a maximum value given by limit.
    """  
    
    if abs(slope*time)> abs(limit):
        return limit
    else:
        return slope*time
...
def pd_series(time_elapsed, uFwd, fwdtOn, fwdtOff, fwdRamp, uRev, revtOn, revtOff,revRamp, n, freq, amp):
	"""
     [added by GVR]
    
     Generate the potential profile for a pulsed deposition series
	
     Sequence is:
         [uFwd for fwdtOn -- off for fwdtOff -- uRev for revtOn -- off for revtOff]  * n    
         
	Keyword arguments:
	time_elapsed -- the elapsed time
	uFwd         -- forward deposition potential
	fwdtOn       -- forward deposition time on [s]
	fwdtOff      -- forward deposition time off [s]
	uRev         -- reverse stripping potential
	revtOn       -- reverse stripping time on [s]
	revtOff      -- reverse stripping time off [s]
	n            -- the number of QUARTER cycles
	
	Returns the potential as a function of the elapsed time; if the elapsed time exceeds the end of the PD sweep, returns None.
	"""
    
    
	off = 0
	#Period
	T = fwdtOn + fwdtOff + revtOn + revtOff
	log_message("elapsed = %s" % str(time_elapsed))
	i = time_elapsed % T # current cycle number

	if time_elapsed < T*n/4:
        
		c = time_elapsed%T
		if c > fwdtOn + fwdtOff + revtOn: # time for reverse off-time
			u = off
		elif c > fwdtOn + fwdtOff:        # time for reverse on-time
			t  = c-(fwdtOn + fwdtOff)    # time elapsed in reverse on state 
			u = ramp(revRamp,t,uRev)
		elif c > fwdtOn:                  # time for forward off-time
			u = off
		else:                             # time for forward on-time
			t  = c                       # time elapsed in forward on-state 
			u = ramp(fwdRamp,t,uFwd)
		
		u = u +sine(c,amp,freq)
		return u, i
        
	else:
		return None, i # PD finished
...
def pd_getparams():
	"""Retrieve the PD arameters from the GUI input fields and store them in a global dictionary. If succesful, return True."""
	global pd_parameters
	try:
		pd_parameters['fwdPotential'] = float(pd_forwardPotential_entry.text())
		pd_parameters['fwdRamp']      = float(pd_forwardPotentialRamp_entry.text())
		pd_parameters['fwdTimeOn']    = float(pd_forwardTimeOn_entry.text())
		pd_parameters['fwdTimeOff']   = float(pd_forwardTimeOff_entry.text())
		pd_parameters['revPotential'] = float(pd_reversePotential_entry.text())
		pd_parameters['revRamp']      = float(pd_reversePotentialRamp_entry.text())
		pd_parameters['revTimeOn']    = float(pd_reverseTimeOn_entry.text())
		pd_parameters['revTimeOff']   = float(pd_reverseTimeOff_entry.text())
		pd_parameters['numcycles']    = float(pd_numcycles_entry.text())
		pd_parameters['numsamples']   = int(pd_numsamples_entry.text())
		pd_parameters['filename']     = str(pd_file_entry.text())   
		pd_parameters['frequency']     = float(pd_acFreq_entry.text())
		pd_parameters['amplitude']     = float(pd_acAmp_entry.text())
...
def pd_validate_parameters():
	"""Check if the chosen PD parameters make sense. If so, return True."""
	
    
#	if pd_parameters['fwdRamp'] > pd_parameters['fwdPotential']/pd_parameters['fwdTimeOn']:
#		QtGui.QMessageBox.critical(mainwidget, "PD error", "The forward ramp is too steep for the on-time")
#		return False
#	if pd_parameters['revRamp'] > pd_parameters['revPotential']/pd_parameters['revTimeOn']:
#		QtGui.QMessageBox.critical(mainwidget, "PD error", "The reverse ramp is too steep for the on-time")
#		return False
	if pd_parameters['fwdTimeOn'] <0:
		QtGui.QMessageBox.critical(mainwidget, "PD error", "The forward on-time must be greater than 0")
		return False
	#if pd_parameters['revTimeOn']+pd_parameters['revTimeOff'] <= 0:
#		QtGui.QMessageBox.critical(mainwidget, "PD error", "The sum of the reverse on-time and off-time must be greater than 0")
		return False
	return True
...
def cd_update():
	...
	if cd_currentcycle > cd_parameters['numcycles']: # End of charge/discharge measurements
		...
		if len(cd_time_data.samples) == 0 and len(cd_time_data.averagebuffer) > 0: # A new average was just calculated
			...
			charge = np.abs(scipy.integrate.cumtrapz(cd_current_data.averagebuffer,cd_time_data.averagebuffer,initial=0.)/3600.) # Cumulative charge in Ah
		...
		if (cd_currentsetpoint > 0 and potential > cd_parameters['ubound']) or (cd_currentsetpoint < 0 and potential < cd_parameters['lbound']): # A potential cut-off has been reached
			...
			cd_charges.append(np.abs(np.trapz(cd_current_data.averagebuffer,cd_time_data.averagebuffer)/3600.)) # Cumulative charge in Ah
...
def pd_start():
	"""Initialize the pulsed deposition/stripping measurement."""
	global pd_time_data, pd_potential_data, pd_current_data, pd_potential_plot_curve, pd_current_plot_curve, pd_outputfile,pd_currentcycle, state, skipcounter
	    
	if check_state([States.Idle,States.Stationary_Graph]) and pd_getparams() and pd_validate_parameters() and validate_file(pd_parameters['filename']):		
       
		pd_outputfile = open(pd_parameters['filename'], 'w', 1) # 1 means line-buffered
		pd_outputfile.write("Elapsed time(s)\tPotential(V)\tCurrent(A)\n")
		cd_currentcycle = 1
		set_output(0, pd_parameters['fwdPotential'])
		set_control_mode(False) # Potentiostatic control
		hardware_manual_control_range_dropdown.setCurrentIndex(0) # Start at highest current range
		set_current_range()
		time.sleep(.1) # Allow DAC some time to settle
		pd_time_data = AverageBuffer(pd_parameters['numsamples']) # Holds averaged data for elapsed time
		pd_potential_data = AverageBuffer(pd_parameters['numsamples']) # Holds averaged data for potential
		pd_current_data = AverageBuffer(pd_parameters['numsamples']) # Holds averaged data for current
		set_cell_status(True) # Cell on
		time.sleep(.1) # Allow feedback loop some time to settle
		read_potential_current()
		time.sleep(.1)
		...
		plot_frame.setLabel('bottom', 'Time',      units="s")
		plot_frame.setLabel('left',   'Potential', units="V")
		plot_frame.setLabel('right',  'Current',   units="mA")
        
		pd_potential_plot_curve = plot_frame.plot(pen='y', name='Potential (V)')
		pd_current_plot_curve   = plot_frame.plot(pen='r', name='Current (mA)')

		log_message("PD measurement started. Saving to: %s"%pd_parameters['filename'])
		state = States.Measuring_PD
		skipcounter = 2 # Skip first two data points to suppress artifacts
		pd_parameters['starttime'] = timeit.default_timer()
        		


def pd_update():
	"""Add a new data point to the pulsed deposition measurement (should be called regularly)."""
	global state, skipcounter
	elapsed_time = timeit.default_timer()-pd_parameters['starttime']  
	
	pd_output, pd_currentcycle = pd_series(elapsed_time, 
                                            pd_parameters['fwdPotential'], 
                                            pd_parameters['fwdTimeOn'], 
                                            pd_parameters['fwdTimeOff'], 
                                            pd_parameters['fwdRamp'], 
                                            pd_parameters['revPotential'], 
                                            pd_parameters['revTimeOn'], 
                                            pd_parameters['revTimeOff'], 
                                            pd_parameters['revRamp'], 
                                            pd_parameters['numcycles'],
                                            pd_parameters['frequency'],
                                            pd_parameters['amplitude'])
	pd_current_cycle_entry.setText("%d"%pd_currentcycle) # Indicate the current cycle number
	if pd_output == None: # This signifies the end of the PD process
		pd_stop(interrupted=False)
	else:
		set_output(0, pd_output) # Output a new potential value
		read_potential_current() # Read new potential and current
		if skipcounter == 0: # Process new measurements
			pd_time_data.add_sample(elapsed_time)
			pd_potential_data.add_sample(potential)
			pd_current_data.add_sample(current) 
			if len(pd_time_data.samples) == 0 and len(pd_time_data.averagebuffer) > 0: # Check if a new average was just calculated
				pd_outputfile.write("%e\t%e\t%e\n"%(pd_time_data.averagebuffer[-1],pd_potential_data.averagebuffer[-1],pd_current_data.averagebuffer[-1])) # Write it out
				pd_potential_plot_curve.setData( pd_time_data.averagebuffer,pd_potential_data.averagebuffer) # Update the graph
				pd_current_plot_curve.setData(   pd_time_data.averagebuffer,pd_current_data.averagebuffer) # Update the graph
            
			skipcounter = auto_current_range() # Update the graph
		else: # Wait until the required number of data points is skipped
			skipcounter -= 1

def pd_stop(interrupted=True):
	"""Finish the CV measurement."""
	global state
	if check_state([States.Measuring_PD]):
		set_cell_status(False) # Cell off
		pd_outputfile.close()
		if interrupted:
			log_message("PD measurement interrupted.")
		else:
			log_message("PD measurement finished.")
		state = States.Stationary_Graph # Keep displaying the last plot until the user clicks a button
		preview_cancel_button.show()

def pd_preview():
	"""Generate a preview of the PD potential profile in the plot window, based on the PD parameters currently entered in the GUI."""
	global state
	global ts
	#if check_state([States.Idle,States.Stationary_Graph]) and pd_getparams() and pd_validate_parameters():
	if check_state([States.Stationary_Graph,States.NotConnected,States.Idle]) and pd_getparams() and pd_validate_parameters():
		#log_message("PD preview...")   
		time_arr = [] # Initialize time array
		potential_arr = [] # Initialize potential array
		cycleDuration = pd_parameters['fwdTimeOn']+pd_parameters['fwdTimeOff']+pd_parameters['revTimeOn']+ pd_parameters['revTimeOff'] # Automatic timestep calculation, resulting in 100 potential steps between lower and upper bound
		timeval = 0 
		pd_val, pd_cycle = pd_series(timeval, 
                                      pd_parameters['fwdPotential'], 
                                      pd_parameters['fwdTimeOn'], 
                                      pd_parameters['fwdTimeOff'], 
                                      pd_parameters['fwdRamp'], 
		...
		while pd_val != None: 
				...
				pd_val, pd_cycle = pd_series(timeval, 
                                               ...
                                               pd_parameters['frequency'],
                                               pd_parameters['amplitude'])
				potential_arr.append(pd_val)
				time_arr.append(timeval)		
				log_message('%s percent complete' % str(100*timeval/(cycleDuration*pd_parameters['numcycles'])))
#		try:
#			legend.scene().removeItem(legend)
#		except AttributeError:
#			pass
		...
		plot_frame.enableAutoRange()
		preview_cancel_button.show()
		state = States.Stationary_Graph # Keep displaying the PD preview until the user clicks a button
...
def rate_update():
	...
	if (rate_halfcycle_countdown%2 == 0 and potential > rate_parameters['ubound']) or (rate_halfcycle_countdown%2 != 0 and potential < rate_parameters['lbound']): # A potential cut-off has been reached
		...
		if rate_halfcycle_countdown == 1: # Last charge cycle for this C-rate, so calculate and plot the charge capacity
			charge = np.abs(scipy.integrate.trapz(rate_current_data.averagebuffer,rate_time_data.averagebuffer)/3600.) # Charge in Ah
...
tab_names = ["Hardware","CV","Charge/disch.","Rate testing","Deposit/Strip"]
...
# Set up the GUI - deposit/strip tab
pd_vbox = QtGui.QVBoxLayout()
pd_vbox.setAlignment(QtCore.Qt.AlignTop)

pd_params_box = QtGui.QGroupBox(title="AC Voltammetry", flat=False)
format_box_for_parameter(pd_params_box)
pd_params_layout = QtGui.QVBoxLayout()
pd_params_box.setLayout(pd_params_layout)
pd_forwardPotential_entry = make_label_entry(pd_params_layout, "Forward deposition potential (V)")
pd_forwardPotential_entry.setText("1.0")
pd_forwardPotentialRamp_entry = make_label_entry(pd_params_layout, "    Ramp(V/s) [0 for none]")
pd_forwardPotentialRamp_entry.setText("1.0")
pd_forwardTimeOn_entry =    make_label_entry(pd_params_layout, "Forward deposition time on (s)")
pd_forwardTimeOn_entry.setText("1.0")
pd_forwardTimeOff_entry =   make_label_entry(pd_params_layout, "Forward deposition time off (s)")
pd_forwardTimeOff_entry.setText("1.0")
pd_reversePotential_entry = make_label_entry(pd_params_layout, "Reverse stripping potential (V)")
pd_reversePotential_entry.setText("-1.0")
pd_reversePotentialRamp_entry = make_label_entry(pd_params_layout, "    Ramp(V/s) [0 for none]")
pd_reversePotentialRamp_entry.setText("-1.0")
pd_reverseTimeOn_entry =    make_label_entry(pd_params_layout, "Reverse stripping time on (s)")
pd_reverseTimeOn_entry.setText("1.0")
pd_reverseTimeOff_entry =   make_label_entry(pd_params_layout, "Reverse stripping time off (s)")
pd_reverseTimeOff_entry.setText("1.0")

pd_acFreq_entry =    make_label_entry(pd_params_layout, "Sine frequency")
pd_acFreq_entry.setText("0.1")
pd_acAmp_entry =   make_label_entry(pd_params_layout, "Sine amplitude")
pd_acAmp_entry.setText("0.1")




pd_numcycles_entry = make_label_entry(pd_params_layout, "Number of quarter cycles")
pd_numcycles_entry.setText("4.0")
pd_numsamples_entry = make_label_entry(pd_params_layout, "Samples to average")
pd_numsamples_entry.setText("1")

pd_params_layout.setSpacing(6)
pd_params_layout.setContentsMargins(3,10,3,3)
pd_vbox.addWidget(pd_params_box)

pd_file_box = QtGui.QGroupBox(title="Output data filename", flat=False)
format_box_for_parameter(pd_file_box)
pd_file_layout = QtGui.QVBoxLayout()
pd_file_box.setLayout(pd_file_layout)
pd_file_choose_layout = QtGui.QHBoxLayout()
pd_file_entry = QtGui.QLineEdit()
pd_file_choose_layout.addWidget(pd_file_entry)
pd_file_choose_button = QtGui.QPushButton("...")
pd_file_choose_button.setFixedWidth(32)
pd_file_choose_layout.addWidget(pd_file_choose_button)
pd_file_choose_button.clicked.connect(lambda: choose_file(pd_file_entry,"Choose where to save the pulsed deposition measurement data"))
pd_file_layout.addLayout(pd_file_choose_layout)
pd_file_layout.setSpacing(6)
pd_file_layout.setContentsMargins(3,10,3,3)
pd_vbox.addWidget(pd_file_box)


pd_preview_button = QtGui.QPushButton("Preview sweep")
pd_preview_button.clicked.connect(pd_preview)
pd_vbox.addWidget(pd_preview_button)
pd_start_button = QtGui.QPushButton("Start process")
pd_start_button.clicked.connect(pd_start)
pd_vbox.addWidget(pd_start_button)
pd_stop_button = QtGui.QPushButton("Stop process")
pd_stop_button.clicked.connect(lambda: pd_stop(interrupted=True))
pd_vbox.addWidget(pd_stop_button)

pd_vbox.setSpacing(6)
pd_vbox.setContentsMargins(3,3,3,3)

pd_info_box = QtGui.QGroupBox(title="Information", flat=False)
format_box_for_parameter(pd_info_box)
pd_info_layout = QtGui.QVBoxLayout()
pd_info_box.setLayout(pd_info_layout)
pd_current_cycle_entry = make_label_entry(pd_info_layout, "Current half cycle")
pd_current_cycle_entry.setReadOnly(True)

pd_info_layout.setSpacing(6)
pd_info_layout.setContentsMargins(3,10,3,3)
pd_vbox.addWidget(pd_info_box)


tabs[4].setLayout(pd_vbox)
...
def periodic_update(): # A state machine is used to determine which functions need to be called, depending on the current state of the program
	if state == States.Idle_Init:
		...
		rate_update()
	elif state == States.Measuring_PD:
		pd_update()
